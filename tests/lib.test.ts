...
async function lib(id: StartedTestContainer, cmd: string) {
	return await exec(id, `bash -c 'source /scripts/lib.sh 2>/dev/null; ${cmd}'`);
}
...
describe("lib.sh unit tests", () => {
	...
	beforeAll(async () => {
		...
		// src/skills
		await exec(id, `mkdir -p ${INSTALL}/src/skills/hello-skill`);
		await exec(
			id,
		...
		await exec(id, `mkdir -p ${INSTALL}/src/commands/nested`);
		await exec(
			id,
		...
		await exec(id, `mkdir -p ${INSTALL}/src/hooks/sample-hook`);
		await exec(
			id,
	...
	describe("log helpers", () => {
		it("log_info outputs the message", async () => {
			const r = await lib(id, `log_info "hello info"`);
			expect(r.output).toContain("hello info");
		});

		it("log_success outputs the message", async () => {
			const r = await lib(id, `log_success "all good"`);
			expect(r.output).toContain("all good");
		});

		it("log_warning outputs the message", async () => {
			const r = await lib(id, `log_warning "be careful"`);
			expect(r.output).toContain("be careful");
		});

		it("log_error outputs the message to stderr", async () => {
			const r = await exec(
				id,
	...
	describe("confirm()", () => {
		it("returns 0 when user inputs y", async () => {
			const r = await exec(
				id,
		...
		it("returns non-zero when user inputs n", async () => {
			const r = await exec(
				id,
		...
		it("returns non-zero for any other input", async () => {
			const r = await exec(
				id,
	...
	describe("detect_shell()", () => {
		it("returns zsh when .zshrc exists", async () => {
			await exec(id, `touch ${HOME}/.zshrc`);
			const r = await lib(id, `detect_shell`);
			expect(r.output).toBe("zsh");
		});

		it("returns bash when only .bashrc exists", async () => {
			await exec(id, `rm -f ${HOME}/.zshrc && touch ${HOME}/.bashrc`);
			const r = await lib(id, `detect_shell`);
			expect(r.output).toBe("bash");
		});

		it("returns bash as default when neither exists", async () => {
			await exec(id, `rm -f ${HOME}/.zshrc ${HOME}/.bashrc`);
			const r = await lib(id, `detect_shell`);
			expect(r.output).toBe("bash");
	...
	describe("detect_llms()", () => {
		it("detects both CLIs when available", async () => {
			const r = await lib(id, `detect_llms`);
		...
		it("warns when claude is missing", async () => {
			await exec(id, `mv /usr/local/bin/claude /usr/local/bin/claude.bak`);
			const r = await lib(id, `detect_llms`);
			expect(r.output).toContain("Claude CLI not found");
			await exec(id, `mv /usr/local/bin/claude.bak /usr/local/bin/claude`);
		});

		it("warns when copilot is missing", async () => {
			await exec(id, `mv /usr/local/bin/copilot /usr/local/bin/copilot.bak`);
			const r = await lib(id, `detect_llms`);
			expect(r.output).toContain("GitHub Copilot CLI not found");
			await exec(id, `mv /usr/local/bin/copilot.bak /usr/local/bin/copilot`);
		});

		it("warns that no skills will be installed when no CLI found", async () => {
			await exec(
				id,
	...
	describe("get_version()", () => {
		it("reads version from package.json with jq", async () => {
			const r = await lib(id, `get_version`);
			expect(r.output).toBe(VERSION);
		});

		it("reads version from package.json without jq", async () => {
		...
		it("returns unknown when package.json is missing", async () => {
			await exec(id, `mv ${INSTALL}/package.json ${INSTALL}/package.json.bak`);
			try {
				const r = await lib(id, `get_version`);
				...
				await exec(
					id,
					`mv ${INSTALL}/package.json.bak ${INSTALL}/package.json`,
				);
			}
		});
	});
	...
	describe("init_registry()", () => {
		it("creates registry.json with version and empty skills", async () => {
			await lib(id, `init_registry`);
		...
		it("overwrites an existing registry", async () => {
			await exec(
				id,
		...
		it("initializes registry.json with an empty hooks.enabled list", async () => {
			await lib(id, `init_registry`);
			const r = await exec(id, `cat ${INSTALL}/registry.json`);
			const registry = JSON.parse(r.output);
	...
	describe("install_skills()", () => {
		it("copies canonical skill to ~/.oh-my-skills/skills/", async () => {
			await exec(id, `rm -rf ${INSTALL}/skills`);
			await lib(id, `init_registry && install_skills`);
			...
			const content = await exec(
				id,
				`cat ${INSTALL}/skills/hello-skill/SKILL.md`,
			);
			expect(content.output).toContain("by: oh-my-skills");
		});

		it("creates Claude symlink and updates registry", async () => {
			await exec(id, `rm -rf ${HOME}/.claude/skills`);
			await lib(id, `init_registry && install_skills`);
			...
			const target = await exec(
				id,
				`readlink ${HOME}/.claude/skills/hello-skill`,
			);
		...
		it("generates Copilot wrapper and updates registry", async () => {
			await exec(id, `rm -rf ${HOME}/.copilot/skills`);
			await lib(id, `init_registry && install_skills`);
		...
		it("skips claude wrapper when claude CLI is absent", async () => {
			await exec(
				id,
		...
		it("resets skills list on each call (no duplicates in registry)", async () => {
			await lib(id, `init_registry && install_skills`);
			await lib(id, `install_skills`);
			const r = await exec(id, `cat ${INSTALL}/registry.json`);
			const registry = JSON.parse(r.output);
	...
	describe("registry_read_paths()", () => {
		it("does not fail on an empty skills list with jq (set -e safe)", async () => {
			await lib(id, `init_registry`); // skills.claude/copilot start out as []
			const r = await exec(
				id,
		...
		it("does not fail on an empty skills list without jq (set -e safe)", async () => {
			await lib(id, `init_registry`);
			await exec(id, `mv /usr/bin/jq /usr/bin/jq.bak`);
			try {
				const r = await exec(
	...
	describe("hooks registry", () => {
		it("registry_write_skills preserves an existing hooks.enabled list", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
		...
		it("registry_add_enabled_hook is idempotent", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
		...
		it("registry_remove_enabled_hook removes only the named hook", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
			await lib(id, `registry_add_enabled_hook "other-hook"`);
			await lib(id, `registry_remove_enabled_hook "handoff"`);
		...
		it("registry_read_enabled_hooks prints one name per line", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
			await lib(id, `registry_add_enabled_hook "other-hook"`);

			const r = await lib(id, `registry_read_enabled_hooks`);
			expect(r.output.split("\n").sort()).toEqual(["handoff", "other-hook"]);
		});

		it("registry_write_skills preserves hooks via non-jq fallback", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
		...
		it("registry_read_enabled_hooks does not fail on an empty list with jq (set -e safe)", async () => {
			await lib(id, `init_registry`); // hooks.enabled starts out as []
		...
		it("registry_read_enabled_hooks does not fail on an empty list without jq (set -e safe)", async () => {
			await lib(id, `init_registry`);
		...
		it("registry_read_enabled_hooks works via non-jq sed fallback", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
			await lib(id, `registry_add_enabled_hook "other-hook"`);
		...
		it("registry_add_enabled_hook fails safely when jq is absent", async () => {
			await lib(id, `init_registry`);

			// Hide jq
			await exec(id, `mv /usr/bin/jq /usr/bin/jq.bak`);
			try {
				const r = await lib(id, `registry_add_enabled_hook "handoff"`);
				expect(r.exitCode).not.toBe(0);
				// Registry should still be intact
				const reg = await exec(id, `cat ${INSTALL}/registry.json`);
		...
		it("registry_remove_enabled_hook fails safely when jq is absent", async () => {
			await lib(id, `init_registry`);
			await lib(id, `registry_add_enabled_hook "handoff"`);
			...
			await exec(id, `mv /usr/bin/jq /usr/bin/jq.bak`);
			try {
				const r = await lib(id, `registry_remove_enabled_hook "handoff"`);
				expect(r.exitCode).not.toBe(0);
	...
	describe("settings_merge_hook() / settings_remove_hook()", () => {
		it("creates ~/.claude/settings.json when it doesn't exist", async () => {
			await exec(id, `rm -rf ${HOME}/.claude/settings.json`);
			await lib(
				id,
			...
			const r = await exec(
				id,
				`test -f ${HOME}/.claude/settings.json && echo ok`,
			);
			expect(r.output).toBe("ok");
		});

		it("writes the expected hook entry shape", async () => {
			await exec(id, `rm -f ${HOME}/.claude/settings.json`);
			await lib(
				id,
				`settings_merge_hook "UserPromptSubmit" "*" "/opt/hook.sh" 10`,
			);
			const r = await exec(id, `cat ${HOME}/.claude/settings.json`);
		...
		it("is idempotent — re-merging the same command doesn't duplicate it", async () => {
			await exec(id, `rm -f ${HOME}/.claude/settings.json`);
			await lib(
				id,
				`settings_merge_hook "UserPromptSubmit" "*" "/opt/hook.sh" 10`,
		...
		it("preserves pre-existing unrelated hook entries for the same event", async () => {
			await exec(
				id,
		...
		it("preserves pre-existing entries for other events", async () => {
			await exec(
				id,
		...
		it("aborts without writing when existing settings.json has invalid JSON", async () => {
			await exec(id, `printf 'not json{' > ${HOME}/.claude/settings.json`);
			const r = await lib(
				id,
				`settings_merge_hook "UserPromptSubmit" "*" "/opt/hook.sh" 10`,
			);
			expect(r.exitCode).not.toBe(0);
			const content = await exec(id, `cat ${HOME}/.claude/settings.json`);
			expect(content.output).toBe("not json{");
		});

		it("settings_remove_hook removes only the matching command", async () => {
			await exec(id, `rm -f ${HOME}/.claude/settings.json`);
			await lib(
				id,
				`settings_merge_hook "UserPromptSubmit" "*" "/opt/hook.sh" 10`,
			...
			await lib(id, `settings_remove_hook "UserPromptSubmit" "/opt/hook.sh"`);
			const r = await exec(id, `cat ${HOME}/.claude/settings.json`);
			const settings = JSON.parse(r.output);
			const commands = settings.hooks.UserPromptSubmit.flatMap((g: any) =>
				g.hooks.map((h: any) => h.command),
		...
		it("settings_remove_hook is a no-op success when settings.json doesn't exist", async () => {
			await exec(id, `rm -f ${HOME}/.claude/settings.json`);
			const r = await lib(
				id,
		...
		it("settings_remove_hook only strips the matching hook from a co-located group, not the whole group", async () => {
		...
		it("settings_merge_hook handles a matcher-group with no hooks key at all", async () => {
		...
		it("settings_merge_hook handles a matcher-group with an empty hooks array", async () => {
			await exec(
				id,
		...
		it("settings_remove_hook handles a matcher-group with no hooks key at all", async () => {
		...
		it("settings_remove_hook handles a matcher-group with an empty hooks array", async () => {
			await exec(
				id,
		...
		it("settings_merge_hook's idempotent re-merge only strips the matching hook from a co-located group, not the whole group", async () => {
			await exec(
				id,
				`echo '{"hooks":{"UserPromptSubmit":[{"matcher":"*","hooks":[{"type":"command","command":"/opt/hook.sh","timeout":10},{"type":"command","command":"/my/own/script.sh"}]}]}}' > ${HOME}/.claude/settings.json`,
			);
	...
	describe("hook_enable() / hook_disable()", () => {
		it("hooks_list_available lists canonical hook names", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await lib(id, `install_hooks`);
			const r = await lib(id, `hooks_list_available`);
			expect(r.output.split("\n")).toContain("sample-hook");
		});

		it("hook_enable errors for an unknown hook name", async () => {
			const r = await lib(id, `hook_enable "nonexistent-hook"`);
			expect(r.exitCode).not.toBe(0);
		});

		it("hook_enable merges into settings.json and updates the registry", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks ${HOME}/.claude/settings.json`);
			await lib(id, `install_hooks && init_registry`);
			const r = await lib(id, `hook_enable "sample-hook"`);
			expect(r.exitCode).toBe(0);
			...
			const registry = JSON.parse(
				(await exec(id, `cat ${INSTALL}/registry.json`)).output,
			);
			expect(registry.hooks.enabled).toContain("sample-hook");
		});

		it("hook_disable removes the settings.json entry and the registry record", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks ${HOME}/.claude/settings.json`);
			await lib(
				id,
				`install_hooks && init_registry && hook_enable "sample-hook"`,
			);
			await lib(id, `hook_disable "sample-hook"`);
		...
		it("disable_all_hooks disables every currently-enabled hook", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks ${HOME}/.claude/settings.json`);
			await lib(
				id,
				`install_hooks && init_registry && hook_enable "sample-hook"`,
			);
			await lib(id, `disable_all_hooks`);
		...
		it("hook_enable fails cleanly without jq", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await lib(id, `install_hooks && init_registry`);
			await exec(id, `mv /usr/bin/jq /usr/bin/jq.bak`);
			const r = await lib(id, `hook_enable "sample-hook"`);
		...
		it("hook_enable returns non-zero (not a false success) when the registry write fails", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks ${HOME}/.claude/settings.json`);
			await lib(id, `install_hooks && init_registry`);
			...
			await exec(id, `rm -f ${INSTALL}/registry.json`);
			const r = await lib(id, `hook_enable "sample-hook"`);
			expect(r.exitCode).not.toBe(0);
		...
		it("hook_disable returns non-zero (not a false success) when the registry write fails", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks ${HOME}/.claude/settings.json`);
			await lib(
				id,
				`install_hooks && init_registry && hook_enable "sample-hook"`,
			...
			await exec(id, `rm -f ${INSTALL}/registry.json`);
			const r = await lib(id, `hook_disable "sample-hook"`);
			expect(r.exitCode).not.toBe(0);
	...
	describe("install_commands()", () => {
		it("copies command files to COMMANDS_DIR", async () => {
			await exec(id, `rm -rf ${INSTALL}/commands`);
			await lib(id, `install_commands`);

			const r = await exec(id, `test -f ${INSTALL}/commands/hi.sh && echo ok`);
			expect(r.output).toBe("ok");
		});

		it("copies nested command files", async () => {
			await lib(id, `install_commands`);
			const r = await exec(
				id,
		...
		it("makes .sh files executable", async () => {
			await lib(id, `install_commands`);
			const r = await exec(id, `test -x ${INSTALL}/commands/hi.sh && echo ok`);
			expect(r.output).toBe("ok");
		});

		it("excludes non-.sh files from installation", async () => {
			await exec(id, `rm -rf ${INSTALL}/commands`);
			await lib(id, `install_commands`);
	...
	describe("create_shell_sourcing()", () => {
		it("creates an executable shell file", async () => {
			await lib(id, `create_shell_sourcing "install"`);
			const r = await exec(id, `test -x ${INSTALL}/shell && echo ok`);
			expect(r.output).toBe("ok");
		});

		it("shell file sources commands recursively", async () => {
			await lib(id, `create_shell_sourcing "install"`);
			const content = await exec(id, `cat ${INSTALL}/shell`);
			expect(content.output).toContain("commands");
			expect(content.output).toContain("source");
		});

		it("shell file triggers auto-check on interactive shell", async () => {
			await lib(id, `create_shell_sourcing "install"`);
			const content = await exec(id, `cat ${INSTALL}/shell`);
		...
		it('outputs "created" message in install mode', async () => {
			const r = await lib(id, `create_shell_sourcing "install"`);
			expect(r.output).toContain("created");
		});

		it('outputs "updated" message in update mode', async () => {
			const r = await lib(id, `create_shell_sourcing "update"`);
	...
	describe("inject_sourcing()", () => {
		it("injects source line into .bashrc", async () => {
			await exec(id, `printf '# clean\n' > ${HOME}/.bashrc`);
			await lib(
				id,
				`create_shell_sourcing "install" && inject_sourcing "bash" "install"`,
			);
			const r = await exec(id, `grep "oh-my-skills" ${HOME}/.bashrc`);
			expect(r.exitCode).toBe(0);
			expect(r.output).toContain("source");
		});

		it("injects source line into .zshrc", async () => {
			await exec(id, `printf '# clean\n' > ${HOME}/.zshrc`);
			await lib(
				id,
				`create_shell_sourcing "install" && inject_sourcing "zsh" "install"`,
			);
			const r = await exec(id, `grep "oh-my-skills" ${HOME}/.zshrc`);
			expect(r.exitCode).toBe(0);
			expect(r.output).toContain("source");
		});

		it("preserves existing content when injecting", async () => {
			await exec(
				id,
				`printf '# my config\nexport FOO=bar\n' > ${HOME}/.bashrc`,
			);
		...
		it("does not duplicate sourcing line on repeated calls", async () => {
			await exec(id, `printf '# clean\n' > ${HOME}/.bashrc`);
			await lib(
				id,
				`create_shell_sourcing "install" && inject_sourcing "bash" "install"`,
			);
			await lib(id, `inject_sourcing "bash" "install"`);
			const r = await exec(id, `grep -c "oh-my-skills" ${HOME}/.bashrc`);
			expect(r.output).toBe("1");
		});

		it("silently skips in update mode when sourcing already present", async () => {
			await exec(id, `printf '# clean\n' > ${HOME}/.bashrc`);
			await lib(
				id,
				`create_shell_sourcing "install" && inject_sourcing "bash" "install"`,
			);
			const r = await lib(id, `inject_sourcing "bash" "update"`);
	...
	describe("install_hooks()", () => {
		it("copies canonical hook.json and hook.sh to ~/.oh-my-skills/hooks/", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await lib(id, `install_hooks`);
			...
			const content = await exec(
				id,
				`cat ${INSTALL}/hooks/sample-hook/hook.json`,
			);
			expect(content.output).toContain("UserPromptSubmit");
		});

		it("makes hook.sh executable", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await lib(id, `install_hooks`);
			const r = await exec(
				id,
		...
		it("excludes non hook.json/hook.sh files from installation", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await lib(id, `install_hooks`);
			const r = await exec(
				id,
		...
		it("skips hook directories without hook.json", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await exec(id, `mkdir -p ${INSTALL}/src/hooks/incomplete-hook`);
			await exec(
				id,
			...
			await exec(id, `rm -rf ${INSTALL}/src/hooks/incomplete-hook`);
		});

		it("preserves existing unrelated files under HOOKS_DIR (e.g. .state/)", async () => {
			await exec(id, `rm -rf ${INSTALL}/hooks`);
			await exec(id, `mkdir -p ${INSTALL}/hooks/.state`);
			await exec(
				id,
				`printf 'marker' > ${INSTALL}/hooks/.state/handoff-nudged-xyz`,
			);
		...
		it("does nothing when no src/hooks directory exists", async () => {
			await exec(id, `mv ${INSTALL}/src/hooks ${INSTALL}/src/hooks.bak`);
			const r = await lib(id, `install_hooks`);
			expect(r.exitCode).toBe(0);
			await exec(id, `mv ${INSTALL}/src/hooks.bak ${INSTALL}/src/hooks`);
		});
	});
});
