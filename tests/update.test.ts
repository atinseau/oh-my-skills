...
describe("oh-my-skills Update (real script)", () => {
  ...
  beforeAll(async () => {
    ...
    exec(id, "cd /tmp/remote-repo && git add . && git commit -m 'v0.1.0' && git tag v0.1.0");
  ...
  it("should detect update when remote has new tag", () => {
    ...
    exec(id, "cd /tmp/remote-repo && echo update > newfile && git add . && git commit -m 'v0.2.0' && git tag v0.2.0");

    // Now update should detect a difference
    // Pipe "n" to decline the update (just check detection)
    const r = exec(id, `echo n | REPO_URL=/tmp/remote-repo bash /scripts/update.sh`);
  ...
  it("should update when user confirms", () => {
    ...
    const r = exec(id, `echo y | REPO_URL=/tmp/remote-repo bash /scripts/update.sh`);
    expect(r.exitCode).toBe(0);
