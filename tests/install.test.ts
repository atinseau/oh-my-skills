...
describe("oh-my-skills Install (real script)", () => {
  ...
  beforeAll(async () => {
    ...
    // Commit
    exec(id, "cd /tmp/remote-repo && git add . && git commit -m 'initial' && git tag v0.1.0");
  ...
  it("should have installed skills for Claude", () => {
    ...
    const check = exec(id, `test -f ${HOME}/.claude/skills/greeting-skill/SKILL.md && echo ok`);
    expect(check.output).toBe("ok");

    const check2 = exec(id, `test -f ${HOME}/.claude/skills/deploy-skill/SKILL.md && echo ok`);
  ...
  it("should have installed skills for Copilot", () => {
    ...
    const check = exec(id, `test -f ${HOME}/.copilot/skills/greeting-skill/SKILL.md && echo ok`);
    expect(check.output).toBe("ok");
  });

  it("should have oh-my-skills marker in installed skills", () => {
    const r = exec(id, `grep "by: oh-my-skills" ${HOME}/.claude/skills/greeting-skill/SKILL.md && echo found`);
    expect(r.output).toContain("found");
  });

  it("should have copied commands", () => {
    const r = exec(id, `test -f ${INSTALL}/commands/greet.sh && test -f ${INSTALL}/commands/deploy.sh && echo ok`);
    expect(r.output).toBe("ok");
  });
  ...
  it("should source commands dynamically via shell script", () => {
    const r = exec(id, `bash -c 'shopt -s expand_aliases; source ${INSTALL}/shell && alias greet && alias deploy'`);
    expect(r.exitCode).toBe(0);
