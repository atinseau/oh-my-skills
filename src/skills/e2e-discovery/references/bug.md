...
**How to detect:** Read `{discovery_root}/scenarios/_index.md`. If the scenario exists and has status `tested` with a spec file → Phase 2. Otherwise → Phase 1.
...
- If the bug can't be reproduced → report what was observed, **stop**
...
- Write the `{spec_suffix}` file with assertions for the **expected** (correct) behavior
- Run `{test_command}`
...
1. **Identify the source**
   ...
   - If user pastes test output → parse the failing assertion and locate the spec file in `{test_dir}`
...
5. **Read relevant snapshots** — check the map's States table for snapshots matching the failing state. This gives UI structure context without opening a browser. Useful as a **reference for what the UI should look like**, not as a substitute for live observation.
...
**How to distinguish:** open the page with `playwright-cli open [url]`, replay the scenario steps, snapshot and compare with what the test asserts. If the UI is correct but the test fails → test bug. If the UI is wrong → app bug.
...
1. **Update the scenario** — status → `covered` in both the scenario file and `_index.md`. Update Coverage Summary counts. Add a note: "Fixed in [commit hash] — [brief description]"
2. **Update the map** (if the fix changed the UI) — update `{discovery_root}/map/`, set `Last validated` to today
3. **Report**
